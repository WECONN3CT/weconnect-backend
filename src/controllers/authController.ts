...
import { validatePassword, validateEmail } from '../middleware/security';

// JWT Secret - MUSS in Produktion gesetzt sein!
const JWT_SECRET = process.env.JWT_SECRET;
const JWT_EXPIRES_IN = process.env.JWT_EXPIRES_IN || '1d'; // Standard: 1 Tag (sicherer als 7d)
...
const generateToken = (userId: string): string => {
  return jwt.sign({ userId }, JWT_SECRET as string, { expiresIn: JWT_EXPIRES_IN } as jwt.SignOptions);
};
...
export const signup = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // E-Mail-Format prüfen (strenge Validierung)
